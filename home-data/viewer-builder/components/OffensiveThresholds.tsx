...
import type {
  OffensiveSafeMarginEntry,
  OffensiveConstrainer,
  OffensiveUpgradeEntry,
  OffensiveUpgrader,
} from "../spAnalysisCalc";
...
/** Format defender's defensive investment: e.g. "B32+補正" or "D8" */
function formatDefInvestment(
  c: { category: string; defenderDefensiveSP: number; defenderNatMod: number },
  lang: Lang,
): string {
  const statChar = c.category === "Physical" ? "B" : "D";
  const boost = c.defenderNatMod >= 1.1
    ? (lang === "ja" ? "+補正" : "+Nat")
    : c.defenderNatMod <= 0.9
      ? (lang === "ja" ? "-補正" : "-Nat")
      : "";
  return `${statChar}${c.defenderDefensiveSP}${boost}`;
}

type SubTab = "reduction" | "upgrade";

interface Props {
  safeMargins: OffensiveSafeMarginEntry[];
  upgrades: OffensiveUpgradeEntry[];
  lang: Lang;
}

export function OffensiveThresholds({ safeMargins, upgrades, lang }: Props) {
      ...
      {subTab === "reduction" && (
        hasReduction ? (
                  ...
                  {!allOK && m.constrainers.length > 0 && (
                    <div className="divide-y divide-gray-700/50 max-w-2xl max-h-[28rem] overflow-y-auto">
                      {m.constrainers.map((c, i) => (
                        <ConstrainerRow key={`${c.defenderName}-${c.moveName}`} c={c} isFirst={i === 0} lang={lang} />
                      ))}
                    </div>
                  )}
                </div>
          ...
          <div className="text-green-400 py-2">
            {lang === "ja"
              ? "攻撃SPに制約なし — 自由に削減可能"
              : "No offensive SP constraints — safe to reduce"}
          </div>
        )
      )}
                ...
                <div key={u.stat} className="rounded border border-gray-700 bg-gray-800/40 overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-700/50 max-w-2xl">
                    {u.upgraders.map((up, i) => (
                      <UpgraderRow key={`${up.defenderName}-${up.moveName}`} u={up} isFirst={i === 0} lang={lang} />
                    ))}
                  </div>
                </div>
              );
...
/** Row for a constrainer (SP reduction worsens KO) */
function ConstrainerRow(
  { c, isFirst, lang }: { c: OffensiveConstrainer; isFirst: boolean; lang: Lang },
) {
  ...
  return (
    <div className={`flex items-center gap-1.5 px-3 py-1.5 ${rowBg}`}>
      ...
      <span className={`shrink-0 truncate max-w-[7rem] ${
        ...
        {localizePokemon(c.defenderName, lang)}
      </span>

      <span className={`text-[10px] shrink-0 font-mono ${
        c.defenderNatMod >= 1.1 ? "text-orange-400"
        : c.defenderDefensiveSP >= 16 ? "text-sky-400"
        : "text-gray-300"
      }`}>
        {formatDefInvestment(c, lang)}
      </span>
      ...
      <span className={`text-[10px] shrink-0 w-6 text-center ${
        c.speedComparison === "faster" ? "text-green-400"
        : c.speedComparison === "slower" ? "text-red-400"
        : "text-gray-500"
      }`}>
      ...
      <span className="text-[10px] text-gray-600 shrink-0 w-10 text-right">
        {c.defenderUsagePct.toFixed(1)}%
      </span>
    </div>
  );
}

/** Row for an upgrader (SP increase improves KO) */
function UpgraderRow(
  { u, isFirst, lang }: { u: OffensiveUpgrader; isFirst: boolean; lang: Lang },
) {
  ...
  return (
    <div className={`flex items-center gap-1.5 px-3 py-1.5 ${rowBg}`}>
      ...
      <span className={`shrink-0 truncate max-w-[7rem] ${
        ...
        {localizePokemon(u.defenderName, lang)}
      </span>

      <span className={`text-[10px] shrink-0 font-mono ${
        u.defenderNatMod >= 1.1 ? "text-orange-400"
        : u.defenderDefensiveSP >= 16 ? "text-sky-400"
        : "text-gray-300"
      }`}>
        {formatDefInvestment(u, lang)}
      </span>
      ...
      <span className={`text-[10px] shrink-0 w-6 text-center ${
        u.speedComparison === "faster" ? "text-green-400"
        : u.speedComparison === "slower" ? "text-red-400"
        : "text-gray-500"
      }`}>
